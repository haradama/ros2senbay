...
class BaseX:
    ...
    def decodeLongValue(self, sVal):
        ...
        for i in reversed(range(len(sVal))):
            v = math.pow(self.PN, len(sVal)-i-1) * \
                int(self.REVERSE_TABLE[ord(sVal[i])])
            totalVal += v
...
class SenbayFormat:
            ...
            if len(contents) > 1:
                ...
                if len(val) > 0:
                    if val[:1] != "'":
                        if isReservedKey:
                            encodedText = '%s%s%s' % (
                                encodedText, key, self.basex.encodeDoubleValue(float(val)))
                        else:
                            encodedText = '%s%s:%s' % (
                                encodedText, key, self.basex.encodeDoubleValue(float(val)))
                    else:
    ...
    def decode(self, text):
        ...
        for element in elements:
            ...
            if val[:1] != "'":
                decodedText = '%s%s:%s' % (
                    decodedText, key, self.basex.decodeDoubleValue(val))
            else:
